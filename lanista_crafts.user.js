...
(function () {
	...
	// "Liten sten" (type_name "ranged") is the one base ranged weapon in the game - every
	// Distansvapen loadout is that same cheap item, its own damage near-negligible, optionally
	// carrying one attached enchant (Slunga, Kastyxa, ...) that does the real work. Confirmed
	// live: that enchant isn't a separate top-level equipped entry, it's nested under this
	// item's own `enchants[].enchant` - and it's the enchant, not "Liten sten" itself, that the
	// build simulator's Distansvapen slot models (it only knows the 24 catalogued
	// restrict_to_distance enchant items, matched by name same as everything else here). A bare
	// "Liten sten" with nothing enchanted onto it has nothing to hand over for that slot.
	function buildSimRangedEnchantName(item) {
		const enchant = item.enchants && item.enchants[0] && item.enchants[0].enchant;
		return enchant ? enchant.name : null;
	}

	function buildSimEquippedEntry(item) {
		const slot = buildSimSlotForItem(item);
		if (!slot) return null;
		if (slot === 'ranged') {
			const name = buildSimRangedEnchantName(item);
	...
	function buildMatchTable(battles) {
		...
		// A bare <tr> inside a <thead> has its own default gradient too (confirmed live -
		// rgb(54,44,28) -> rgb(43,34,20), warm like tbody's own default, just lighter) -
		// stripping surface-table-header off the <thead> element itself doesn't touch this
		// <tr>, which is a separate element with its own default. Same fix as the body rows:
		// an inline override, since no class can beat a bare-element selector.
		headerRow.style.backgroundImage = 'none';
		...
		battles.forEach((battle) => {
			...
			// Confirmed live: a bare, classless <tr> inside any <tbody> on this site already
			// renders with this same warm linear-gradient by default (a global tbody tr style,
			// not something surface-row itself sets - unlike thead's gradient, which really was
			// tied to surface-table-header and went away when that class was dropped). No class
			// removal can beat that, so this overrides it directly - inline styles win against
			// any stylesheet selector regardless of specificity.
